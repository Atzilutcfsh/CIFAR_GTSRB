...
if args.poison_type == 'dynamic':
    ...
    if args.dataset == 'cifar10':
        ...
        train = datasets.CIFAR10(os.path.join(data_dir, 'cifar10'), train=True,
                                     download=True, transform=data_transform)
                
        random.seed(2333) 
        subset_idx = random.sample(list(range(len(train))), num_samples)
        train_set = torch.utils.data.Subset(train, subset_idx)
        ...
        num_samples = args.num_sample
        train = datasets.GTSRB(os.path.join(data_dir, 'gtsrb'), split = 'train',
        ...
        #filtered_indices = [i for i in range(len(train)) if train[i][1] in class_indices]
        #train_set = torch.utils.data.Subset(train, filtered_indices)
