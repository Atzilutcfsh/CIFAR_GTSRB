...
create_poisoned_set() {
  local poison_type=$1
  local poison_rate=$2
  python create_poisoned_set_8.py -dataset="$dataset" -poison_type="$poison_type" -poison_rate="$poison_rate" -exp_name="$exp_name" -num_sample="$num_sample"
}
...
#python mainc.py -m train -g 0 -s "$num_shadow" -e "$exp_name" -n clean
